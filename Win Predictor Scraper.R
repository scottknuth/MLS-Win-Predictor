...
#Cut out duplicates in Catcher2
...
#Catcher 4
Catcher4 <- data.frame(Player = character(), PPG = as.numeric())

for (i
     in Catcher3$Starting_11) {
  WS4 <- read_html(i)
  Player <- WS4 %>% html_nodes("h1 span") %>% html_text() %>% as.character()
  PPG <- WS4 %>% html_nodes("#stats_playing_time_dom_lg .group_start:nth-child(16)") %>% html_text() %>% as.numeric()
  temp4 <- data.frame(Player, PPG)
